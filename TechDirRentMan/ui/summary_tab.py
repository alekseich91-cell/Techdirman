...
def ungroup_selected_items(page: Any, zone_key: str) -> None:
    """
    Разъединяет выделенные позиции и возвращает их в состояние без группы.

    Вместо присвоения каждой строке уникального имени группы (что приводило
    к автоматическому окрашиванию строк и скрытой агрегации), данная
    реализация присваивает всем выбранным позициям служебное имя группы
    ``"Аренда оборудования"``. Это значение является значением по умолчанию
    для поля ``group_name`` в базе данных и трактуется интерфейсом как
    отсутствие пользовательской группы. В результате строки становятся
    «не сгруппированными» и отображаются без цвета.

    :param page: Экземпляр ProjectPage
    :param zone_key: Ключ зоны, в которой выполняется разбиение
    """
    # Получаем таблицу по ключу зоны; если таблица отсутствует, выходим
    table = page.zone_tables.get(zone_key)
    if not isinstance(table, QtWidgets.QTableWidget):
        return
    # Собираем уникальные индексы выбранных строк
    selected_rows = sorted({idx.row() for idx in table.selectedIndexes()})
    if not selected_rows:
        return
    updated_count = 0
    # Проходим по каждой выбранной строке и сбрасываем group_name
    for r in selected_rows:
        itm = table.item(r, 0)
        if itm is None:
            continue
        # id строки хранится в пользовательских данных
        item_id_data = itm.data(QtCore.Qt.ItemDataRole.UserRole)
        try:
            item_id = int(item_id_data)
        except Exception:
            # пропускаем некорректный id
            continue
        # Сбрасываем group_name в служебное значение «Аренда оборудования»,
        # которое обозначает отсутствие группы
        try:
            page.db.update_item_fields(item_id, {"group_name": "Аренда оборудования"})
            updated_count += 1
        except Exception:
            # логируем ошибку обновления, но продолжаем обработку остальных строк
            try:
                if hasattr(page, "_log"):
                    page._log(f"Ошибка разъединения позиции id={item_id}")
            except Exception:
                pass
            continue
    # Записываем информационное сообщение в лог о количестве разъединённых позиций
    try:
        if updated_count > 0 and hasattr(page, "_log"):
            page._log(f"Разъединение: {updated_count} позиций возвращены в состояние без группы.")
    except Exception:
        pass
    # Перезагружаем таблицы зон, чтобы отобразить обновлённые данные
    try:
        page._reload_zone_tabs()
    except Exception:
        pass
...
def reload_zone_tabs(page: Any) -> None:
    ...
    for zone_key, table in page.zone_tables.items():
        ...
        if snap_mode:
            ...
            # Агрегатор для группировки строк сводной сметы. Ранее ключ
            # строился из нормализованного наименования, подрядчика, зоны,
            # отдела и имени группы. Это приводило к автоматическому
            # объединению одинаковых позиций даже без явного добавления в
            # группу. Чтобы отображать каждую строку отдельно до тех пор,
            # пока пользователь не объединит их, используем в качестве
            # ключа уникальный идентификатор позиции. Таким образом,
            # каждая запись остаётся самостоятельной, а поля group_name
            # и прочие атрибуты используются только для отображения цвета
            # и информации, но не влияют на агрегацию.
            ...
            for r in rows:
                ...
                # отключена, и объединение происходит только при явном
                # назначении группы пользователем.
                try:
                    # Получаем уникальный идентификатор позиции. sqlite3.Row
                    # поддерживает доступ по ключу как словарь, но не метод get().
                    item_id_key = int(r["id"])
                except Exception:
                    # В случае ошибки оставляем исходное значение идентификатора
                    # (например, если не приводится к int).
                    try:
                        item_id_key = r["id"]
                    except Exception:
                        item_id_key = None
